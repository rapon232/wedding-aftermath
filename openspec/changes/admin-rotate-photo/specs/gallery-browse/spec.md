...
The lightbox SHALL offer admins a single rotate control on eligible photos (JPEG/PNG/WebP originals, processing complete). Each activation SHALL rotate the stored original 90° clockwise, regenerate the thumbnail and preview renditions, and update the item's dimensions — the change is permanent and applies to what every guest sees and downloads. Cached copies in browsers SHALL be bypassed after a rotation. Non-admins SHALL NOT see the control, and the endpoint SHALL reject them.
...
- **WHEN** the lightbox shows a video, or a photo whose original is HEIC or GIF
