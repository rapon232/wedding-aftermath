...
test('creating a guest with an existing name is skipped (no duplicate)', async () => {
  const first = await req(srv.base, 'POST', '/api/admin/guests', {
    cookie: admin,
    json: { names: ['Unique Uma'] },
  });
  assert.equal(first.data.length, 1);
  const dupe = await req(srv.base, 'POST', '/api/admin/guests', {
    cookie: admin,
    json: { names: ['unique uma', 'Brand New'] },
  });
...
test('create a single guest with name + email (hand-add without CSV)', async () => {
  ...
  const dupe = await req(srv.base, 'POST', '/api/admin/guests', {
    cookie: admin,
    json: { name: 'Sam Twin', email: 'sam@example.com' },
  });
  assert.equal(dupe.data.length, 0);
  // an email alongside multiple names is rejected
  const bad = await req(srv.base, 'POST', '/api/admin/guests', {
    cookie: admin,
    json: { names: ['A B', 'C D'], email: 'x@example.com' },
  });
...
test('make-admin grants and revokes, but never removes the last admin', async () => {
  ...
  assert.equal(
    (await req(srv.base, 'POST', `/api/admin/guests/${id}/admin`, { cookie: admin, json: { isAdmin: true } }))
      .status,
    200,
  );
  ...
  const r = await req(srv.base, 'POST', `/api/admin/guests/${admins[0].id}/admin`, {
    cookie: admin,
    json: { isAdmin: false },
  });
...
test('activation stamps on first login and shows in the guest list', async () => {
  ...
  const before = (await req(srv.base, 'GET', '/api/admin/guests', { cookie: admin })).data.find(
    (x) => x.id === g.data[0].id,
  );
  assert.equal(before.activated_at, null, 'not activated before login');
  await login(srv.base, code);
  const after = (await req(srv.base, 'GET', '/api/admin/guests', { cookie: admin })).data.find(
    (x) => x.id === g.data[0].id,
  );
...
test('send-invite guards: no email → 400, email-not-configured → 503, non-admin → 403', async () => {
  const noEmail = await req(srv.base, 'POST', '/api/admin/guests', {
    cookie: admin,
    json: { names: ['Emailless'] },
  });
  ...
  const imp = await req(srv.base, 'POST', '/api/admin/import', {
    cookie: admin,
    json: { csv: 'name,email\nCarol,carol@example.com\n' },
  });
  const r2 = await req(srv.base, 'POST', `/api/admin/guests/${imp.data.created[0].id}/invite`, {
    cookie: admin,
  });
  ...
  const r3 = await req(srv.base, 'POST', `/api/admin/guests/${imp.data.created[0].id}/invite`, {
    cookie: guest,
  });
